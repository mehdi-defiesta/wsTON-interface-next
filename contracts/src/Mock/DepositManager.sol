...
contract DepositManager is ProxyStorage, AccessibleCommon, DepositManagerStorage {
    ...
    function initialize(address wton_, address registry_, address seigManager_, uint256 globalWithdrawalDelay_)
        external
    {
    ...
    function _decodeDepositManagerOnApproveData(bytes memory data) internal pure returns (address layer2) {
    ...
    function _redeposit(address layer2, uint256 i, uint256 n) internal returns (bool) {
        ...
        require(
            _withdrawalRequests[layer2][msg.sender].length - i >= n, "DepositManager: n exceeds num of pending requests"
        );
    ...
    function withdrawalRequest(address layer2, address account, uint256 index)
        external
        view
        returns (uint128 withdrawableBlockNumber, uint128 amount, bool processed)
    {
