...
contract RefactorCoinageSnapshot is ProxyStorage, AuthControlCoinage, RefactorCoinageSnapshotStorage, DSMath {
    ...
    event ChangedBalance(
        address indexed account,
        IRefactor.Balance oldBalance,
        IRefactor.Balance newBalance,
        IRefactor.Balance oldTotalBalance,
        IRefactor.Balance newTotalBalance
    );
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(currentAccountBalance >= amount && currentTotalBalance >= amount, "insufficient balance");
    ...
    function _applyFactorAt(IRefactor.Balance memory _balance, IRefactor.Factor memory _factor)
        internal
        pure
        returns (uint256)
    {
    ...
    function balanceOfAt(address account, uint256 snapshotId) external view returns (uint256 amount) {
        (IRefactor.Balance memory _balance, IRefactor.Factor memory _factor) =
            getBalanceAndFactorAt(account, snapshotId);
        amount = _applyFactorAt(_balance, _factor);
    }
    ...
    function getBalanceAndFactor(address account)
        public
        view
        returns (IRefactor.Balance memory, IRefactor.Factor memory)
    {
        return (_valueAtAccountBalanceLast(account), _valueAtFactorLast());
    }

    function getTotalAndFactorAt(uint256 snapshotId)
        public
        view
        returns (IRefactor.Balance memory, IRefactor.Factor memory)
    {
    ...
    function getBalanceAndFactorAt(address account, uint256 snapshotId)
        public
        view
        returns (IRefactor.Balance memory, IRefactor.Factor memory)
    {
    ...
    function _valueAtTotalSupplyLast() internal view returns (IRefactor.Balance memory) {
        uint256 index = 0;
    ...
    function _valueAtFactorLast() internal view returns (IRefactor.Factor memory) {
        uint256 index = 0;
    ...
    function _valueAtAccountBalanceLast(address account) internal view returns (IRefactor.Balance memory) {
        uint256 index = 0;
    ...
    function _valueAtTotalSupply(uint256 snapshotId) internal view returns (IRefactor.Balance memory balance) {
    ...
    function _valueAtFactor(uint256 snapshotId) internal view returns (IRefactor.Factor memory factor_) {
        require(snapshotId <= progressSnapshotId(), "snapshotId > progressSnapshotId");
        uint256 index = factorSnapshotIds.findValue(snapshotId);
        return factorSnapshots[index];
    }

    function _valueAtAccount(uint256 snapshotId, address account)
        internal
        view
