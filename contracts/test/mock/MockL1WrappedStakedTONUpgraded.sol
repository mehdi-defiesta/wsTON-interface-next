...
contract MockL1WrappedStakedTONUpgraded is
    ProxyStorage,
    ERC20Upgradeable,
    OwnableUpgradeable,
    ReentrancyGuard,
    L1WrappedStakedTONStorage
{
    ...
    function _depositAndGetWSTONTo(address _to, uint256 _amount, bool _token) internal returns (bool) {
        ...
        if (!_token) {
            ...
            require(IDepositManager(depositManager).deposit(layer2Address, _amount), "deposit failed");
            ...
            require(ITON(ton).approveAndCall(wton, _amount, data), "approveAndCall failed");
